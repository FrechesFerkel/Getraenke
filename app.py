...
with col2:
    raw_data = load_data()
    if raw_data:
        processed_list = [{"Tester": r.get("Profiles", {}).get("name", "Unbekannt"), "Getränk": r.get("drink_aName", "Unbenannt"), "Punkte": r.get("rating", 0), "Fazit": r.get("remark", ""), "Bild": r.get("image_url", None)} for r in raw_data]
        ...
        with t1:
            ...
            def add_badges(row):
                ...
                if df[df["Tester"] == name]["Bild"].count() > 0: badge_str += " 📸" # Hat Bilder
                if df[df["Tester"] == name]["Punkte"].max() == 10: badge_str += " 🔥" # Hat eine 10 gegeben
                return badge_str
