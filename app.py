...
# Individuelles CSS für Team Tasting
st.markdown("""
    <style>
...
def create_radar_chart(row_data):
    """Einzel-Radar für die Fotowall (3 Achsen)."""
    ...
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, 10], gridcolor="#444"), angularaxis=dict(gridcolor="#444")),
...
def create_comparison_radar(df, drink_a, drink_b):
    """Doppel-Radar für den Duell-Modus."""
    categories = ['Geschmack', 'Design', 'Preis-Leistung']
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        theta=categories + [categories[0]], fill='toself', name=drink_a, line_color='#ff4b4b', fillcolor='rgba(255, 75, 75, 0.2)'
    ))
    # Herausforderer 2
    fig.add_trace(go.Scatterpolar(
        r=[stats_b['taste'], stats_b['design'], stats_b['vibe'], stats_b['taste']],
        theta=categories + [categories[0]], fill='toself', name=drink_b, line_color='#00f2ff', fillcolor='rgba(0, 242, 255, 0.2)'
    ))
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, 10], gridcolor="#444")),
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=380, margin=dict(l=50, r=50, t=40, b=40)
    )
    return fig

# --- 2. SIDEBAR (Alle Features: Löschen, Orakel, Export, Filter) ---
with st.sidebar:
    st.header("👤 Profil")
    ...
    if raw_export_data:
        export_list = [{"Tester": r.get("Profiles", {}).get("name"), "Getränk": r.get("drink_aName"), "Punkte": r.get("rating"), "Geschmack": r.get("taste"), "Design": r.get("design"), "Preis-Leistung": r.get("vibe")} for r in raw_export_data]
        export_df = pd.DataFrame(export_list)
        csv = export_df.to_csv(index=False).encode('utf-8')
        st.download_button(label="📥 Liste herunterladen", data=csv, file_name=f"tasting_export.csv", mime="text/csv")
    ...
    filter_user = st.selectbox("Fotowall nur von:", options=["Alle"] + tester_list)
        ...
        with st.form("rating_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Speichern 🚀"):
                if final_drink:
                    ...
                    action = save_entry(user, final_drink, rating, comment, c_design, c_taste, c_price, img_url)
                    if action == "updated": st.info(f"🔄 Rating für {final_drink} wurde aktualisiert!")
                    else: st.success(f"✅ Neues Rating gespeichert!")
                    st.rerun()
                else:
                    st.warning("Bitte Getränkenamen eingeben!")
    else:
        st.warning("👈 Gib links deinen Namen ein, um zu starten.")

with col2:
    raw_data = load_data()
    if raw_data:
        processed_list = [{"id": r.get("id"), "Tester": r.get("Profiles", {}).get("name"), "Getränk": r.get("drink_aName"), "Punkte": r.get("rating"), "taste": r.get("taste"), "design": r.get("design"), "vibe": r.get("vibe"), "Fazit": r.get("remark"), "Bild": r.get("image_url")} for r in raw_data]
        df = pd.DataFrame(processed_list)

        # --- METRIKEN: WALL OF FAME & SHAME ---
        ...
        with t2:
            ...
            for r in reversed(display_list):
                with st.container():
                    ...
                    with c_radar:
                        st.plotly_chart(create_radar_chart(r), use_container_width=True, key=f"radar_{r['id']}")
                    with c_txt:
        ...
        with t3:
            st.write("### Beliebtheit der Getränke")
        ...
        with t4:
            ...
            if len(all_drinks_list) >= 2:
                ...
                if drink_1 != drink_2:
                    ...
                    s1 = df[df['Getränk'] == drink_1]['Punkte'].mean()
                    s2 = df[df['Getränk'] == drink_2]['Punkte'].mean()
                    st.markdown(f"**Score:** {drink_1}: `{s1:.1f}` vs. {drink_2}: `{s2:.1f}`")
                else:
                    st.warning("Wähle zwei verschiedene Getränke!")
    else:
